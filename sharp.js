...
fs.readdirSync(target).forEach((image) => {
  if (image.includes('hero')) {
    // convert hero image width 1200px
    sharp(`${target}/${image}`)
      .resize(1200)
      .toFile(
        path.resolve(
    ...
    // convert hero image width 1000px
    sharp(`${target}/${image}`)
      .resize(1000)
      .toFile(
        path.resolve(
    ...
    // convert hero image width 600px
    sharp(`${target}/${image}`)
      .resize(600)
      .toFile(
        path.resolve(
    ...
    // convert loading image width 400px
    sharp(`${target}/${image}`)
      .resize(400)
      .toFile(
        path.resolve(
    ...
    // convert loading image width 300px
    sharp(`${target}/${image}`)
      .resize(300)
      .toFile(
        path.resolve(
    ...
    // convert loading image width 200px
    sharp(`${target}/${image}`)
      .resize(200)
      .toFile(
        path.resolve(
